...
def home(request):
    ...
    if 'id' in request.session.keys():
        print("The request owner is ", request.session['type'])
        if request.session['type'] == 'customer':
            shoplist = product.objects.all().order_by('-ordercount')[:5]
            ...
            return render(request, 'shopmenu/userhome.html', context)

        elif request.session['type'] == 'shop':
            shop = Shop.objects.get(email=request.session['id'])
            query = Order.objects.order_by('-pk').all()
            dic = {}
            customer = {}
            for x in query:
                if x.busid == shop.email:
                    dic2 = {}
                    if (x.deliverystatus == 'd'):
                        continue
                    x.calamount()
                    for i, j in zip(x.getproductitems(), x.getqty()):
                        dic2[i] = j
                    dic[x] = dic2
            ...
            return render(request, 'shopmenu/resthome.html', context)
    else:
        return render(request, "shopmenu/login.html",context)
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print("The request is from someone whose is ", request.session['type'], "with email", request.session['id'])
            ...
            try:
                #shop= get_object_or_404(Shop, email=email)
                shop=Shop.objects.get(email=email)

                if shop.check_password(password.encode('utf-8')):
                    request.session['id'] = email
...
def signup(request):
    if request.method == 'POST':
        ...
        if usertype == 'Customer':
            ...
            user = Shop(name=name, email=email, phone=phone, address=address)
            user.set_password(user.make_password(password.encode('utf-8')))
            user.save()
            request.session['id'] = email
            request.session['type'] = 'shop'
    ...
    if request.method == 'GET':
        return render(request, 'shopmenu/login.html')
...
def details(request):
    ...
    if 'id' in request.session.keys():
        if request.session['type'] == 'customer':
            ...
            return render(request, 'shopmenu/userdetails.html', context)
        else:

            if request.session['type'] == 'shop':
                context = {
                    'shop': Shop.objects.get(email=request.session['id'])
                }
                return render(request, 'shopmenu/restdetails.html', context)
     ...
     if request.method == 'POST':
         if request.session['type'] == 'customer':
             ...
             if 'pic' in request.FILES:
                ...
                filename = str(request.POST.get('pic'))
                image=request.FILES["pic"]
                # server_name = __getCleanFilename(filename)
                server_name = (customer.name) + os.path.sep + filename
                customer.image.save(server_name, image)
             ...
             phone = request.POST.get('phone')
             address = request.POST.get('address')
             res_type = request.POST.get('bussiness_type')
             ...
             return render(request, 'shopmenu/details.html')


# For checking the image in the product
...
# 2-Filtering filename string
def __getCleanFilename(filename):
    # directory = self.profile.user.username
    customer = Customer.objects.get(email=request.session['id']).name
    name = customer + os.path.sep + filename
    return name
...
def addProduct(request):
    ...
    return render(request, 'shopmenu/addProduct.html', context)


def addproductitem(request):
    print("Request session is " ,request.session)
    print("Request session keys is " ,request.session['id'])
    print("Request session type is " ,request.session['type'])


    if 'id' in request.session.keys() and request.session['type'] == 'shop':
        shop = Shop.objects.get(email=request.session['id'])
        name = request.POST['name']
        cuisine = request.POST['cuisine']
        price = request.POST['price']
        print("The request files are ", request.POST)
        # img=request.POST['image']
        if 'pic' in request.FILES:
            print("The request files are ", request.FILES)

            possim = request.FILES['pic']
            filename = str(request.FILES['pic'])
            # server_name = __getCleanFilename(filename)

            server_name = (shop.name) + os.path.sep + filename

        Product = product(busid=shop, name=name, cuisine=cuisine, price=price,
                        availability_time=datetime.datetime.now())
        Product.image.save(server_name, possim)
        Product.save()
     ...
     if 'id' in request.session.keys():
         try:
             ...
             for x in allProducts:
                 if x.busid.email ==shop.email:
                     try:
                         menuitems[x.cuisine].append(x)
                     except KeyError:
                         menuitems[x.cuisine] = [x]
             context = {
                 'customer': customer,
                 'shop': shop,
                 'productitems': menuitems,
             }
             return render(request, 'shopmenu/shopview.html', context)
         ...
         if request.method == 'GET':
             ...
             for x in query:
                 if x.customer.email == customer.email:
                     amount = amount + x.product.price * x.qty
                     try:
                         cart[x.product.busid].append(x)
                     except KeyError:
                         cart[x.product.busid] = [x]
             ...
             return render(request, "shopmenu/ordercart.html", context)
             ...
             for q in Cart.objects.all():
                 if q.customer == Customer.objects.get(email=request.session['id']):
                     try:
                         orders[q.product.busid] = orders[q.product.busid] + ',' + str(q.product.pk)
                     except KeyError:
                         orders[q.product.busid] = str(q.product.pk)
                     try:
                         ordersqty[q.product.busid] = ordersqty[q.product.busid] + ',' + str(q.qty)
                     except KeyError:
                         ordersqty[q.product.busid] = str(q.qty)
                     q.delete()
             for x, y in zip(orders, ordersqty):
                 o = Order(customer=customer, Shop=x, shopist=orders[x], qty=ordersqty[y],
                           ordertime=datetime.datetime.now(), deliverystatus='p')
                 o.calamount()
                 o.save()
             messages.success(request, "Payment Successfull :)")
             return render(request, "shopmenu/ordercart.html")
     else:
         return render(request, "shopmenu/login.html")
...
def saveToCart(request):
     if 'id' in request.session.keys():
         ...
         for x, y in cart.iteritems():
             for z in y:
                 z.product.ordercount = z.product.ordercount + z.qty
             z.product.save()
         return render(request, "shopmenu/ordercart.html", context)
     else:
         return render(request, "shopmenu/login.html")
...
#     # self.profile.profile_pic.save(server_name, picture.open())
#
#
...
#             # server_name = __getCleanFilename(filename)
#
#             server_name = (restaurant.name) + os.path.sep + filename
#
#         food = FoodItem(resid=restaurant, name=name, cuisine=cuisine, price=price, course='s',
#                         availability_time=datetime.datetime.now())
#         food.image.save(server_name, possim)
#         food.save()
